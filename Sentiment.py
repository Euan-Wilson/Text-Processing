...
def testBayes(sentencesTest, dataName, pWordPos, pWordNeg, pWord,pPos):
    ...
    # Call evaluation function to calculate and print the performance metrics for
    # the Naive Bayes classsifier
    evaluation(correct, total, correctpos, totalpospred, totalpos, correctneg, totalnegpred, totalneg)
...
# Function to calculate the performance metrics for the different classifiers
def evaluation(correct, total, correctpos, totalpospred, totalpos, correctneg, totalnegpred, totalneg):
        ...
        print("The positive precision is:", precision_pos)
        print("The positive recall is:", recall_pos)
        print("The negative precision is:", precision_neg)
        print("The negative recall is:", recall_neg)
        print("The positive F-Score is:", F1_Score_pos)
        print("The negative F-Score is:", F1_Score_neg) 
...
# Can just call the evaluation function
# so it calculates and prints the performance metrics when the testDictionary function
# is called
    evaluation(correct, total, correctpos, totalpospred, totalpos, correctneg, totalnegpred, totalneg)  

# Function to factor negation into the rule-based classifier
# A list of possible negations are used
def negation(sentencesTest, dataName, sentimentDictionary, threshold):
    ...
    for sentence, sentiment in sentencesTest.items():
        ...
        score = 0       
        previous = ""
        negations = ["not", "never", "cannot", "n't", "'t", "no", "didn't", "isn't", "don't", "without"]
        for word in words:
            if word in sentimentDictionary:
...
#runs sentiment dictionary based classifier with negation on datasets
negation(sentencesTrain,  "Films (Train Data, Rule-Based)\t", sentimentDictionary, -2)
print("----")
negation(sentencesTest,  "Films  (Test Data, Rule-Based)\t",  sentimentDictionary, -2)
print("------")
negation(sentencesNokia, "Nokia   (All Data, Rule-Based)\t",  sentimentDictionary, -3)
